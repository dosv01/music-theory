...
/* ---------- dados musicais ---------- */
// O seletor continua com 12 botões visuais
const PICKER_NOTES = ["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"];

// Mapeamento inteligente: define se a tônica usa grafia de sustenido ou bemol por padrão
const ROOT_NAMES_MAJOR = ["C", "Db", "D", "Eb", "E", "F", "F#", "G", "Ab", "A", "Bb", "B"];
const ROOT_NAMES_MINOR = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "Bb", "B"];
...
/* ---------- ALGORITMO ENARMÔNICO (Magia da Teoria Musical) ---------- */
// Gera as 7 notas diatônicas garantindo 1 letra para cada grau (sem repetir)
function getSpelledScale(rootName, intervals) {
  const rootLetter = rootName.charAt(0);
  const rootLetterIdx = LETTERS.indexOf(rootLetter);
  
  const acc = rootName.slice(1);
  const accVal = (acc.match(/#/g) || []).length - (acc.match(/b/g) || []).length;
  const rootPitch = NATURAL_PITCHES[rootLetter] + accVal;

  return intervals.map((iv, i) => {
    const targetPitch = (rootPitch + iv) % 12;
    ...
    let name = targetLetter;
    if (diff > 0) name += '#'.repeat(diff);
    else if (diff < 0) name += 'b'.repeat(-diff);

    return { name, pitchClass: (targetPitch + 12) % 12 };
  });
}

// Cria um mapa para repassar a grafia correta para o braço do violão
function getNoteNamesMap(spellingArray) {
  const map = [...PICKER_NOTES]; // Fallback padrão
  spellingArray.forEach(n => { map[n.pitchClass] = n.name; });
  return map;
}

/* ---------- funções geradoras (shapes) ---------- */
...
function avoidOpenString(shape) {
  ...
  if (min <= 0) {
    const shift = Math.ceil((1 - min) / 12) * 12;
...
function buildRelativeShapes(root) {
  const rawAnchors = MAJOR_INTERVALS.map((iv) => ((root + iv) % 12 - LOW_E_CLASS + 12) % 12);
...
function buildPentaBoxes(root, isMinor = false) {
  ...
  const boxes = intervals.map((_, boxIdx) => {
    let n = boxIdx;
    let prevF0 = null;
    const shape = TUNING.map((str) => {
      const t0 = scaleNotes[n % 5];
      const t1 = scaleNotes[(n + 1) % 5];
      n += 2;
      const raw = (t) => (t - str.idx + 12) % 12;
      let f0 = raw(t0);
      let f1 = raw(t1);
      if (prevF0 !== null) {
        while (f0 < prevF0 - 2) f0 += 12;
        while (f0 > prevF0 + 3) f0 -= 12;
      }
      if (f1 <= f0) f1 += 12;
      prevF0 = f0;
      return [f0, f1];
...
function buildField(scaleSpelling, triadQ, romans, seventhQ) {
  return scaleSpelling.map((noteObj, i) => {
    const triadPos = [i, (i + 2) % 7, (i + 4) % 7];
    const seventhPos = [i, (i + 2) % 7, (i + 4) % 7, (i + 6) % 7];
    return {
      roman: romans[i],
      chordRootIdx: noteObj.pitchClass,
      triadName: noteObj.name + qualitySuffix(triadQ[i]),
      triadNotesClasses: triadPos.map(p => scaleSpelling[p].pitchClass),
      triadNotes: triadPos.map(p => scaleSpelling[p].name),
      seventhName: noteObj.name + qualitySuffix(seventhQ[i]),
      seventhNotesClasses: seventhPos.map(p => scaleSpelling[p].pitchClass),
      seventhNotes: seventhPos.map(p => scaleSpelling[p].name),
    };
  });
}
...
export default function CampoHarmonicoModos() {
  ...
  useEffect(() => {
    fetch("https://countapi.mileshilliard.com/api/v1/hit/music-theory")
      ...
      .catch(() => console.log("%c[debug] contador offline", "color:#7C7462"));
  }, []);

  // Nomes raízes amigáveis (ex: Eb no maior, D# no menor)
  const majorRootName = ROOT_NAMES_MAJOR[root];
  const minorRootName = ROOT_NAMES_MINOR[root];

  // Escritas enarmônicas dinâmicas
  const majorSpelling = useMemo(() => getSpelledScale(majorRootName, MAJOR_INTERVALS), [majorRootName]);
  const minorSpelling = useMemo(() => getSpelledScale(minorRootName, MINOR_INTERVALS), [minorRootName]);
  
  // Mapas para passar para o Fretboard desenhar as notas corretamente
  const majorNamesMap = useMemo(() => getNoteNamesMap(majorSpelling), [majorSpelling]);
  const minorNamesMap = useMemo(() => getNoteNamesMap(minorSpelling), [minorSpelling]);

  // Construção dos campos com as notas corretas (sem repetir letras)
  const majorField = useMemo(() => buildField(majorSpelling, MAJOR_TRIAD_Q, MAJOR_ROMAN, MAJOR_7TH_Q), [majorSpelling]);
  const minorField = useMemo(() => buildField(minorSpelling, MINOR_TRIAD_Q, MINOR_ROMAN, MINOR_7TH_Q), [minorSpelling]);

  // Modos Gregos
  const allShapes = useMemo(() => (system === "parallel" ? buildParallelShapes(root) : buildRelativeShapes(root)), [system, root]);
  const mode = MODES[modeIndex];
  const { modeRoot, shapeLowToHigh } = allShapes[modeIndex];
  
  // Enarmonia específica para o Modo selecionado no Fretboard
  const activeModeNamesMap = useMemo(() => {
      if (system === "relative") return majorNamesMap;
      return getNoteNamesMap(getSpelledScale(majorRootName, mode.intervals));
  }, [system, majorNamesMap, majorRootName, mode.intervals]);
  
  const modeNoteNames = mode.intervals.map(iv => activeModeNamesMap[(modeRoot + iv) % 12]);
  
  const shapeByString = useMemo(() => [...shapeLowToHigh].reverse().map(frets => frets.map(f => f + octaveNudge)), [shapeLowToHigh, octaveNudge]);
    ...
    <div className="wrap">
      <style>{`
        ...
        * { box-sizing: border-box; }
        .wrap { background: #15120D; color: #EDE6D6; font-family: 'Inter', system-ui, sans-serif; padding: 0; min-height: 100%; border-radius: 10px; overflow: hidden; }
        .hero { position: relative; padding: 36px 28px 26px; background: repeating-linear-gradient(to bottom, transparent 0px, transparent 17px, rgba(237,230,214,0.10) 17px, rgba(237,230,214,0.10) 18px); background-position: 0 14px; background-size: 100% 90px; border-bottom: 1px solid #3A3226; }
        .eyebrow { font-family: 'IBM Plex Mono', monospace; font-size: 11px; letter-spacing: 0.14em; text-transform: uppercase; color: #C9A227; margin: 0 0 8px; }
        h1 { font-family: 'Spectral', serif; font-weight: 700; font-size: 30px; margin: 0 0 6px; color: #F4EEDD; }
        ...
        .notebtn { font-family: 'IBM Plex Mono', monospace; font-size: 13px; font-weight: 600; padding: 9px 0; width: 42px; text-align: center; border-radius: 6px; border: 1px solid #3A3226; background: #1F1A13; color: #D8CDB8; cursor: pointer; position: relative; transition: transform .12s ease, border-color .12s ease; }
        .notebtn.sharp { background: #17130E; color: #A99C86; }
        .notebtn:hover { transform: translateY(-2px); border-color: #C9A227; }
        .notebtn.active { background: #C9A227; color: #15120D; border-color: #C9A227; }
        .notebtn.active::after { content: ''; position: absolute; bottom: -8px; left: 50%; width: 5px; height: 5px; margin-left: -2.5px; border-radius: 50%; background: #C9A227; }
        section.panel { padding: 26px 28px; border-bottom: 1px solid #221D16; }
        section.panel:last-child { border-bottom: none; }
        .panel h2 { font-family: 'Spectral', serif; font-size: 19px; margin: 0 0 4px; color: #F4EEDD; }
        ...
        .field-card .field-title { font-family: 'IBM Plex Mono', monospace; font-size: 12px; letter-spacing: .08em; text-transform: uppercase; padding: 12px 14px; color: #C9A227; border-bottom: 1px solid #2E271C; }
        ...
        .mode-tab { font-family: 'IBM Plex Mono', monospace; font-size: 12px; padding: 8px 12px; border-radius: 6px; border: 1px solid #2E271C; background: #1B160F; color: #A99C86; cursor: pointer; }
        ...
        .note-chip { font-family: 'IBM Plex Mono', monospace; font-size: 12px; padding: 4px 9px; border-radius: 999px; background: #241F16; border: 1px solid #3A3226; color: #D8CDB8; }
        ...
        .pos-tab { font-family: 'IBM Plex Mono', monospace; font-size: 11px; padding: 6px 10px; border-radius: 5px; border: 1px solid #2E271C; background: #1B160F; color: #7C7462; cursor: pointer; }
      ...
      <section className="panel">
        <h2>Campo Harmônico de {majorRootName} e {minorRootName}</h2>
        ...
        <div className="fields-grid">
          <FieldTable title={`${majorRootName} maior`} rows={majorField} scaleNamesMap={majorNamesMap} onChordClick={setSelectedChord} />
          <FieldTable title={`${minorRootName} menor natural`} rows={minorField} scaleNamesMap={minorNamesMap} onChordClick={setSelectedChord} />
        </div>
      </section>
      ...
      {selectedChord && (
        <section className="panel" style={{ backgroundColor: 'rgba(201, 162, 39, 0.03)' }}>
          ...
          <p className="hint">
            Arpejo mapeando as notas <strong>{selectedChord.notes.join(' - ')}</strong> até a 15ª casa. 
            A nota vermelha é a tônica ({selectedChord.scaleNamesMap[selectedChord.rootIdx]}).
          </p>
          <div className="fretboard-card">
            <Fretboard
              frets={Array.from({length: 16}, (_, i) => i)}
              shapeByString={chordShape}
              rootIdx={selectedChord.rootIdx}
              inlayFrets={inlayFrets}
              noteNamesMap={selectedChord.scaleNamesMap}
            />
          </div>
        </section>
      )}
      ...
      <section className="panel">
        <h2>Pentatônicas de {pentaType === "minor" ? minorRootName : majorRootName}</h2>
        <p className="hint">As 5 caixas clássicas mapeadas pelo braço.</p>
        <div className="mode-tabs" style={{ marginBottom: 16 }}>
          <button className={`mode-tab ${pentaType === "major" ? "active" : ""}`} onClick={() => { setPentaType("major"); setPentaBoxIdx(0); }}>
            Maior<span>{majorRootName} Pentatônica Maior</span>
          </button>
          <button className={`mode-tab ${pentaType === "minor" ? "active" : ""}`} onClick={() => { setPentaType("minor"); setPentaBoxIdx(0); }}>
            Menor<span>{minorRootName} Pentatônica Menor</span>
          </button>
        </div>
        <div className="mode-tabs">
          {pentaBoxes.map((_, i) => (
            <button key={i} className={`mode-tab ${i === pentaBoxIdx ? "active" : ""}`} onClick={() => setPentaBoxIdx(i)}>
              Caixa {i + 1}
            </button>
          ))}
        </div>
        ...
        <div className="fretboard-card">
          <Fretboard
            ...
            rootIdx={root}
            inlayFrets={inlayFrets}
            noteNamesMap={pentaType === "minor" ? minorNamesMap : majorNamesMap}
          />
        </div>
      </section>

      <section className="panel">
        <h2>Modos Gregos de {majorRootName}</h2>
        <div className="mode-tabs" style={{ marginBottom: 16 }}>
          <button className={`mode-tab ${system === "parallel" ? "active" : ""}`} onClick={() => { setSystem("parallel"); setOctaveNudge(0); }}>
            Paralelos<span>mesma tônica ({majorRootName})</span>
          </button>
          <button className={`mode-tab ${system === "relative" ? "active" : ""}`} onClick={() => { setSystem("relative"); setOctaveNudge(0); }}>
            Relativos<span>campo harmônico de {majorRootName}</span>
          </button>
        </div>
        <div className="mode-tabs">
          {MODES.map((m, i) => {
            const mRoot = system === "parallel" ? majorRootName : majorNamesMap[(root + MAJOR_INTERVALS[i]) % 12];
            return (
              <button key={m.name} className={`mode-tab ${i === modeIndex ? "active" : ""}`} onClick={() => { setModeIndex(i); setOctaveNudge(0); }}>
                {mRoot} {m.name} {m.sub && <span>{m.sub}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="pos-row">
          <div className="pos-tabs">
            <button className="pos-tab" disabled={minFret - 12 < 0} style={minFret - 12 < 0 ? { opacity: 0.35, cursor: "default" } : undefined} onClick={() => setOctaveNudge((v) => v - 12)}>← 1 oitava</button>
            <button className="pos-tab" onClick={() => setOctaveNudge(0)}>posição padrão</button>
            <button className="pos-tab" onClick={() => setOctaveNudge((v) => v + 12)}>1 oitava →</button>
          </div>
        ...
        <div className="fretboard-card">
          <Fretboard
            ...
            rootIdx={modeRoot}
            inlayFrets={inlayFrets}
            noteNamesMap={activeModeNamesMap}
          />
        </div>
      </section>
      ...
      <footer className="site-footer">
        <p>
          Mini App Criado com o Claude + Github Pages por dosv01. Aproveita, dá uma moral, e se inscreve nos meus canais:
          <span className="footer-links">
            <a className="footer-link" href="https://www.youtube.com/@DaniloSantosGT" target="_blank" rel="noopener noreferrer">
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M22 8.5s-.2-1.6-.8-2.3c-.8-.9-1.7-.9-2.1-1C16.3 5 12 5 12 5h0s-4.3 0-7.1.2c-.4 0-1.3.1-2.1 1C2.2 6.9 2 8.5 2 8.5S1.8 10.4 1.8 12.2v1.6c0 1.9.2 3.7.2 3.7s.2 1.6.8 2.3c.8.9 1.9.9 2.4 1C6.9 21 12 21 12 21s4.3 0 7.1-.2c.4 0 1.3-.1 2.1-1 .6-.7.8-2.3.8-2.3s.2-1.9.2-3.7v-1.6c0-1.9-.2-3.7-.2-3.7ZM9.8 15.5v-6l5.6 3-5.6 3Z" /></svg> YouTube
            </a>
            <a className="footer-link" href="https://www.instagram.com/dosv01/" target="_blank" rel="noopener noreferrer">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.2" cy="6.8" r="1.1" fill="currentColor" stroke="none" /></svg> Instagram
            </a>
          </span>
        </p>
...
function FieldTable({ title, rows, scaleNamesMap, onChordClick }) {
  return (
    <div className="field-card">
      ...
      <table>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr 
              key={i} className="clickable-row"
              onClick={() => onChordClick({ name: r.seventhName, rootIdx: r.chordRootIdx, notes: r.seventhNotes, notesClasses: r.seventhNotesClasses, scaleNamesMap })}
...
function Fretboard({ frets, shapeByString, rootIdx, inlayFrets, noteNamesMap }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", maxWidth: `${width}px`, height: "auto", display: "block" }}>
      {gridLines.map((lv) => (
        <line key={lv} x1={lineX(lv)} x2={lineX(lv)} y1={topPad} y2={topPad + rowH * (STRINGS_TOP_TO_BOTTOM.length - 1)} stroke={lv === 0 ? "#EDE6D6" : "#4A4130"} strokeWidth={lv === 0 ? 3 : 1.5} />
      ))}
      {STRINGS_TOP_TO_BOTTOM.map((s, r) => (
        <line key={r} x1={lineX(lineStart)} x2={lineX(maxFret)} y1={topPad + r * rowH} y2={topPad + r * rowH} stroke="#6E6656" strokeWidth={1} />
      ))}
      {gridLines.map((f) => {
        if (f < 1 || !inlayFrets.has(f) || (!usedFrets.has(f) && frets.length < 10)) return null;
        return <circle key={f} cx={noteX(f)} cy={topPad + rowH * (STRINGS_TOP_TO_BOTTOM.length - 1) + 16} r={2.5} fill="#3A3226" />;
      })}
      {gridLines.map((f) => {
        if (f < 1 || (!usedFrets.has(f) && frets.length < 10)) return null;
        return <text key={f} x={noteX(f)} y={topPad - 8} fontSize={10} fill="#6E6656" textAnchor="middle">{f}</text>;
      })}
      {STRINGS_TOP_TO_BOTTOM.map((s, r) =>
        shapeByString[r].map((f) => {
          ...
          return (
            <g key={`${r}-${f}`}>
              <circle cx={cx} cy={cy} r={10} fill={isRoot ? "#B5482F" : "#1F1A13"} stroke={isRoot ? "#B5482F" : "#C9A227"} strokeWidth={1.5} />
              <text x={cx} y={cy + 3.5} fontSize={9.5} fontWeight={600} fill={isRoot ? "#F4EEDD" : "#E8B23D"} textAnchor="middle">
                {noteNamesMap[noteIdx]}
              </text>
            </g>
          );
        })
      ...
      {STRINGS_TOP_TO_BOTTOM.map((s, r) => (
        <text key={`label-${r}`} x={leftPad - 16} y={topPad + r * rowH + 3.5} fontSize={11} fill="#7C7462" textAnchor="middle">{s.label}</text>
      ))}
    </svg>
  );
}
